...
ppg_data = pd.read_csv("cleanPPG.csv")
time = ppg_data["VAL"].values
ppg_values = ppg_data["PPG1"].values
# convert the readings from Pandas result to numpy array
# also flatten the 2D array to 1D list
value_list = ppg_values.flatten()
time_list = time.flatten()
...
print("TIME WITH PEAK : ", timeWithPeak)
